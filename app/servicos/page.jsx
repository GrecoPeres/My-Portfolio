          ...
          {services.map((service, index)=> {
            ...
            <div key={index} className='flex-1 flex flex-col justify-center gap-6 group'>
              ...
              <div className='w-full flex justify-between items-center'>
                ...
                <Link href={service.href} className='w-[70px] h-[70px] rounded-full bg-white group-hover:bg-teal-300 transition-all duration-500 flex justify-center items-center hover:-rotate-45'>
                  <BsArrowDownRight className='text-primary text-3xl' />
                </Link>
              </div>
              {/* cabeçalho */}
              <h2 className='text-[42px] font-bold leading-none text-white group-hover:text-teal-300 transition-all duration-500'>{service.titulo}</h2>
              {/* descricao */}
